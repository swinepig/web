...
public class Controller extends HttpServlet{
	...
	/**
	 * 
	 * 处理登录请求http://localhost:8080/friend_demo/userDetail.jsp?id=1&username=rerer&phone=23232323
	 * */
		if(path.equals("/login")){
			String username =request.getParameter("username");
			String pwd =request.getParameter("pwd");
			Service service=Factory.newServiceInstance();
			try {
		 ...
		 * */
			...
			Service service=Factory.newServiceInstance();
			try {
				service.HandleRegist(username, name, pwd, age, sex, phone, number,number2,filepath);
